...
const Profile = () => {
  ...
  return (
    <div className="mx-20 min-h-screen flex flex-col">
      <Navbar />
      <div className="max-w-4xl mx-auto dark:bg-gray-900 dark:border-gray-600 bg-white border border-gray-200 rounded-2xl my-5 p-8">
          ...
          <div className="flex items-center gap-1">
            {user?.profile?.skills.length !== 0 ? (
              user?.profile?.skills.map((item, index) => (
                <Badge className="dark:bg-gray-800 dark:text-gray-300" key={index}>
                  {item}
                </Badge>
              ))
            ) : (
      ...
      {user?.role === "student" && <StudentVerificationCard />}

      <div className="max-w-4xl mx-auto dark:bg-gray-900 rounded-2xl">
